...
module piece() {
    ...
    difference() {
        ...
        // slants
        translate([0, 0, 65+1.75+2+1 + extra_hh])
        rotate([45, 0, 0])
        translate([-d4, -d4/2, 0])
        cube(d4*2);

        translate([0, 0, 65+1.75+2+1+7.5 + extra_hh])
        rotate([-45, 0, 0])
        translate([-d4, -d4/2, 0])
        cube(d4*2);

        // ratchets
        for (z=[3.5:6:hh + extra_hh]) {
...
module pieces_x2() {
    ...
    for (i=[0, 1]) {
        ...
        rotate([0, 0, 90*i]) {
            ...
            // attachments
            for (k=[1, -1]) scale([k, 1, 1]) for (z=[hh/3, hh*.67])
                translate([-d-.7-.2, .25, z])
                rotate([0, 0, 45])
                translate([.5, 0, 0])
                rotate([0, -60, 0])
...
for (i=[0:1]) {
    for (j=[0:1]) {
        translate([d4*1.145*j, d4*1.145*i, 0])
        pieces_x2();
    }
}