...
module p0_new(h=LAYER_HEIGHT,
    ...
    difference() {
        intersection() {
            ...
            // sharpen column
            translate([0, 0, -THICKNESS/4]) union() {
                d2 = h*6;
...
translate([-d2/2, -THICKNESS/2, h*.585])
cube([d2, THICKNESS, h/2]);
...
module min_hinge() {
    hinge_new(nb_layers=2, layer_height=2.5, thickness=5,
              width1=9/2, width2=9/2, angle2=0);
}
