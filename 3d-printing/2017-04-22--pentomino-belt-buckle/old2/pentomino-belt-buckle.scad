...
module make_grip_side(x, y, side) {
    ...
    if (!convex) {
        center = (side=="n") ? [BLOCK_SIDE*.75,     BLOCK_SIDE, ATOM] :
                 (side=="s") ? [BLOCK_SIDE*.25,              0, ATOM] :
                 (side=="e") ? [    BLOCK_SIDE, BLOCK_SIDE*.25, ATOM] :
               /*(side=="w")*/ [             0, BLOCK_SIDE*.75, ATOM];
        color("yellow")
        translate(center)
...
module make_cavity_side(x, y, side) {
    ...
    center = (side=="n") ? [BLOCK_SIDE*.25, BLOCK_SIDE -CAVITY_DISP, -ATOM] :
             (side=="s") ? [BLOCK_SIDE*.75,            +CAVITY_DISP, -ATOM] :
             (side=="e") ? [BLOCK_SIDE -CAVITY_DISP, BLOCK_SIDE*.75, -ATOM] :
           /*(side=="w")*/ [           +CAVITY_DISP, BLOCK_SIDE*.25, -ATOM];
    color("orange")
    translate(center)
