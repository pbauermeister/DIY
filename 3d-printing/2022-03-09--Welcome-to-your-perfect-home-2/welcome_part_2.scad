...
module supports_1(extra1, extra2, extra3) {
    color("red")
    support1_one(extra1, 62.95 +.4  +.4, 0, is_bottom=true);

    color("green")
    support1_one(extra2, 62.95 + .7 +.4, 62.9-.2);

    color("blue")
    support1_one(extra3, 62.95 + .7 +.4, 125.7);
}
...
module support_2_one() {
    difference() {
        color("grey") union() {
            ...
            translate([ATOM*2, ATOM*10, 0])
            hull() {
                translate([0, -48, 45.1 +1])
                cubelet3();
            ...
            hull() {
                ...
                cubelet2();
                translate([x, -48+6, 45.1 -33])
                cubelet2();
                translate([x, -48, 45.1 -33+9])
                cubelet2();
            }
            translate([ATOM*2, 2.40, 0])
            hull() {
                translate([7-4.5, -48, 45.1 +1])
                cubelet3();

                translate([15.2, -48, 45.1 +1])
                cubelet3();

                translate([7 -4.5, -48, 45.1 -9+2.7 +4.5])
                cubelet3();
                translate([15.2, -48, 45.1 -9-15.2+2.7+7])
                cubelet3();
            }

        }
        translate([LAYER_H*2, -LAYER_H0/2, -LAYER_V0])
        plateau_2r();        

        translate([-LAYER_H, -LAYER_H0/2, -LAYER_V0])
        plateau_2r();        
    }
}
...
module part_2() {
    intersection() {
        union() {
            translate([0, 48 -.05, 0]) plateau_1ar();
