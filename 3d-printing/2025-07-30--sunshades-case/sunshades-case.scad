...
module main() {
    intersection() {
        // Base cylinder
        hull() {
            for (x=[ROUNDED_CORNER_RADIUS+5, WIDTH-ROUNDED_CORNER_RADIUS-4]) {
                for (z=[ROUNDED_CORNER_RADIUS, HEIGHT-ROUNDED_CORNER_RADIUS]) {
                    translate([x, 0, z])
                    rotate([90, 0, 0])
                    cylinder(r=ROUNDED_CORNER_RADIUS, h=RADIUS, center=true);
                }
            }
        }
