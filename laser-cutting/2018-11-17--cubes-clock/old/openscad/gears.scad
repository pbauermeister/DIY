...
module _wheel(modul, zahnzahl, hoehe, bohrung, eingriffswinkel,
              schraegungswinkel) {
    pfeilrad(modul, zahnzahl, hoehe, bohrung, eingriffswinkel,
             schraegungswinkel);
}

module gears_check_distance() {
    if (gears_distance()!=GEARS_DISTANCE)
        echo(str("### WARNING: GEARS_DISTANCE should be equal to ",
                 gears_distance(), " but is ", GEARS_DISTANCE));
    else
        echo(str("GEARS_DISTANCE is ", GEARS_DISTANCE, " -- OK"));
}

module gears_wheel(thickness=GEARS_THICKNESS) {
    gears_check_distance() {}
    rotate([0, 0, 360/30/2])
    _wheel(modul=GEARS_MODULE, zahnzahl=30, hoehe=thickness,
           bohrung=SCREW2_DIAMETER, eingriffswinkel=20,
           schraegungswinkel=GEAR_SLANT?20:0);
}
...
module gears_pinion(thickness=GEARS_THICKNESS) {
    gears_check_distance() {}
    _wheel(modul=GEARS_MODULE, zahnzahl=14, hoehe=thickness,
           bohrung=ROD_DIAMETER + TOLERANCE, eingriffswinkel=20,
           schraegungswinkel=GEAR_SLANT?-20:0); 
}
...
function gear_radius(inner=false, median=false,
	...
	let(alpha_stirn = atan(tan(eingriffswinkel)/cos(schraegungswinkel)))
									// Schrägungswinkel im Stirnschnitt
	let(db = d * cos(alpha_stirn))	// Grundkreisdurchmesser
	let(rb = db / 2)					// Grundkreisradius
	let(da = (modul <1)? d + modul * 2.2 : d + modul * 2)
									// Kopfkreisdurchmesser DIN 58400 / DIN 867
